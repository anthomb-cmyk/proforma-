...
import { upsertEvent, deleteEvent } from "./gcalSync.js";
...
// gcalToken + dealName are optional; when provided, syncs to Google Calendar.
export async function createFollowUp(supabase, { dealId, contactId = null, dueAt, note = null, gcalToken = null, dealName = null }) {
  try {
    ...
    // GCal sync — upsert (handles existing event id or null).
    let gcalWarning = null;
    let gcalEventId = null;
    if (gcalToken) {
      const label   = dealName || dealId;
      const gcalRes = await upsertEvent(gcalToken, existing?.gcal_event_id || null, {
        summary:    `Follow-up: ${label}`,
        description: note || "",
        dueAtUtc:   row.due_at
      });
      if (gcalRes.ok) {
        gcalEventId = gcalRes.eventId;
        if (gcalEventId && gcalEventId !== (existing?.gcal_event_id || null)) {
          await supabase.from("follow_ups").update({ gcal_event_id: gcalEventId }).eq("id", row.id);
          row = { ...row, gcal_event_id: gcalEventId };
        }
      } else {
        gcalWarning = `GCal sync failed: ${gcalRes.error}`;
      }
    }

    return { ok: true, data: row, error: null, gcalWarning, gcalEventId };
  } catch (err) {
...
// gcalToken + dealName are optional; when provided, syncs to Google Calendar.
export async function rescheduleFollowUp(supabase, { dealId, dueAt, gcalToken = null, dealName = null }) {
  try {
    // Fetch current row first to get gcal_event_id.
    const { data: existing, error: fetchErr } = await supabase
      .from("follow_ups")
      .select("id, gcal_event_id")
      .eq("deal_id", dealId)
    ...
    if (fetchErr) return { ok: false, data: null, error: fetchErr.message };
    if (!existing) return { ok: false, data: null, error: `No pending follow-up found for deal ${dealId}.` };
...
// gcalToken is optional; when provided, deletes the GCal event.
export async function completeFollowUp(supabase, { dealId, gcalToken = null }) {
  try {
    // Fetch to get gcal_event_id before updating status.
    const { data: existing, error: fetchErr } = await supabase
      .from("follow_ups")
      .select("id, gcal_event_id")
      .eq("deal_id", dealId)
    ...
    let gcalWarning = null;
    if (gcalToken) {
      const gcalRes = await deleteEvent(gcalToken, existing.gcal_event_id);
      if (!gcalRes.ok) gcalWarning = `GCal delete failed: ${gcalRes.error}`;
    }

    return { ok: true, data, error: null, gcalWarning };
  } catch (err) {
...
// gcalToken is optional; when provided, deletes the GCal event.
export async function cancelFollowUp(supabase, { dealId, gcalToken = null }) {
  try {
    // Fetch to get gcal_event_id before updating status.
    const { data: existing, error: fetchErr } = await supabase
      .from("follow_ups")
